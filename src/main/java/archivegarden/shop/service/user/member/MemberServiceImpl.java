...
public class MemberServiceImpl implements MemberService {
    ...
    public Long join(JoinMemberForm form) {
        ...
        Delivery delivery = Delivery.createDeliveryWhenJoin(form.getZipCode(), form.getBasicAddress(), form.getDetailAddress(), member.getName(), member.getPhonenumber());
     ...
     *
     * @param form 회원가입 폼 DTO
     * @throws DuplicateEntityException 이미 존재하는 회원일 경우
     */
    @Override
...
//        smsUtil.sendVerificationCode(to, verificationCode);

        redisUtil.saveData(to, verificationCode, VERIFICATION_CODE_EXPIRE_SECONDS);
    }
     ...
     * @return VerificationCodeResult 인증번호 검증 결과 enum
     */
    @Override
    public VerificationCodeResult verifyVerificationCode(String phonenumber, String verificationCode) {
        if (!redisUtil.existData(phonenumber)) {
            return VerificationCodeResult.EXPIRED;
        }

        String storedVerificationCode = redisUtil.getData(phonenumber);
        if (storedVerificationCode.equals(verificationCode)) {
            return VerificationCodeResult.SUCCESS;
        } else {
            return VerificationCodeResult.MISMATCH;
        }
    }
